<x-app-layout>
    <x-slot name="header">
        <div class="flex items-end justify-between">
            <div>
                ...
                <div>
                    <x-breadcrumb :breadcrumbs="[['title' => __('titles.all_posts'), 'url' => '']]" />
                </div>
            </div>
            @if (auth()->user())
    ...
    <div class="">
        <div x-data="{ open: false }" class="flex justify-end mb-2 relative w-full">
            <div @click="open = !open"
                class="cursor-pointer ring-1 ring-content_text ring-opacity-25 flex items-center justify-center p-1 bg-content rounded-lg hover:bg-content-hover">
            ...
            <div x-show="open" @click.away="open = false"
                ...
                <div class="rounded-md ring-1 ring-content_text ring-opacity-25 py-1 bg-content">
                    @foreach (['en', 'ru', 'pl'] as $lang)
                        ...
                        @else
                            <a href="{{ route('post.showAll.locale', ['locale' => $lang]) }}"
                                class="block w-full px-4 py-1 text-left text-sm leading-5 text-content_text hover:bg-content-hover focus:outline-none focus:bg-content-hover transition duration-150 ease-in-out">
                                @if ($lang == 'ru')
                                    Русский
                                @endif
                                    ...
                                    <div class="flex items-center justify-between flex-wrap">
                                        <div class="flex space-x-3">
                                            ...
                                            <x-icon-with-text icon="like"
                                                ...
                                                tooltip="{{ __('interfaces.likes') }}">{{ $post->likes->count() }}</x-icon-with-text>
                                            <x-icon-with-text icon="eye" fill="#252525"
                                                tooltip="{{ __('interfaces.views') }}">{{ $post->views_count }}</x-icon-with-text>
                                            <x-icon-with-text icon="comments" fill="#252525"
                                                tooltip="{{ __('interfaces.comments') }}">{{ $post->comments_count }}</x-icon-with-text>
                                        </div>
                                        @php
                                            if (app()->getLocale() == 'en') {
                                                $href = route('post.show', ['alias' => $post->alias]);
                                            } else {
                                                $href = route('post.show.locale', [
                                                    'alias' => $post->alias,
