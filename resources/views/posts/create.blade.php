<x-app-layout>
    ...
    <form method="POST" action="{{ route('post.store') }}" enctype="multipart/form-data" id="postCreateForm">
        ...
        <div class="pb-3 mb-3 space-y-1 relative after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-current after:opacity-10">
            <x-input-text id="title" name="title" required label="Title" maxlength="255" value="{{ old('title') }}" description="{{__('interfaces.maxchar_255')}}"/>
        </div>
        ...
        @role('review_editor|administrator')
            <div class="pb-3 mb-3 space-y-1 relative after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-current after:opacity-10">
                <x-input-text id="keywords" name="keywords" value="{{ old('keywords') }}" required label="Keywords" maxlength=255 description="{{__('interfaces.maxchar_255')}}"/>
            </div>
            ...
            <div class="pb-3 space-y-1 relative after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-current after:opacity-10">
                <x-input-text id="seo_description" name="seo_description" value="{{ old('seo_description') }}" label="Description" maxlength="255" description="{{__('interfaces.maxchar_255')}}"/>
            </div>
            ...
            <div class="pb-3 mb-3 space-y-1 relative after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-current after:opacity-10">
                <x-input-text id="language" name="language" required label="Language" maxlength="2" value="{{ old('language') }}" description="en / pl / ru ..."/>
            </div>
        @endrole
