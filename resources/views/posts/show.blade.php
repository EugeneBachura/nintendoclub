    ...
    <x-slot name="header_img">
        <div class="pb-6 px-0 hidden sm:block md:px-6">
            @php
                ...
                if (mb_strlen($post_title) > $limit) {
                    $cutOff = mb_strripos(mb_substr($post_title, 0, $limit), ' ');
                    $trimmed = mb_substr($post_title, 0, $cutOff) . $ending;
                } else {
                    $trimmed = $post_title;
                }
            @endphp 
            <x-breadcrumb :breadcrumbs="[
                ['title' => __('titles.all_posts'), 'url' => localizedRoute('post.showAll')],
                ['title' => $trimmed, 'url' => '']
            ]" />
        </div>
        <div class="flex z-0 justify-center items-center w-full min-h-[120px] sm:min-h-[400px] sm:rounded-t-lg relative" style="background-image: url('{{ asset('storage/posts_images/' . $post->image) }}'); background-size: cover; background-position: center center;">
        ...
        <div class="flex justify-between space-x-4">
            ...
            <div class="pl-3 flex justify-end space-x-1">
                <div class="flex space-x-3">
                    ...
                    <div class="like-container flex justify-end" data-post-id="{{ $post->id }}" data-is-liked="{{ $isLiked ? 'true' : 'false' }}">
                        <x-icon-with-text icon="like" fill="#ff3b3c" class="like-icon active" style="{{ $isLiked ? '' : 'display:none;' }}">
                            <span class="likes-count">{{ $post->likes->count() }}</span>
                        </x-icon-with-text>
                        <x-icon-with-text icon="like" fill="#252525" class="like-icon inactive" style="{{ !$isLiked ? '' : 'display:none;' }}">
                            <span class="likes-count">{{ $post->likes->count() }}</span>
                        </x-icon-with-text>
                    </div>
                    {{-- <x-icon-with-text icon="like" fill="#252525">{{$post->likes}}</x-icon-with-text> --}}
    ...
    <div class="flex flex-col space-y-8 mt-8">
            ...
            @if ($user_level >= 2)
                <div id="comment-form-container" class="mt-2 mx-0 w-full pl-0">
                    <form id="comment_form" action="{{ route('comments.store') }}" method="POST">
                        @csrf
                        <x-textarea id="comment_content" class="block mt-1 -ml-1 w-full" rows="3" name="content" placeholder="{{__('interfaces.leave_comment')}}"></x-textarea>
                    ...
                    document.addEventListener('DOMContentLoaded', function() {
                        document.querySelectorAll('.reply-button').forEach(button => {
                            button.addEventListener('click', function() {
                                ...
                                // Переместить форму к комментарию (опционально)
        ...
        @foreach ($post->comments as $comment)
            @php
                ...
                if ($comment->design()) {
                    $nickname_color = $comment->design()->nickname_color;
                };
            @endphp
            @include('components.comments', ['comment' => $comment, 'depth' => 0, 'nickname_color' => $nickname_color])
        @endforeach
    </div>
    ...
    <script>
        ...
        container.addEventListener('click', function() {
            ...
            .then(data => {
                ...
                this.querySelector('.inactive').style.display = isLiked ? '' : 'none';
                this.setAttribute('data-is-liked', isLiked ? 'false' : 'true');
                this.querySelectorAll('.likes-count').forEach(el => el.textContent = data.likesCount);
            })
