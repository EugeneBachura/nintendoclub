        ...
        <div class="flex items-center content-center space-x-2">
            ...
            <div class="py-2 flex flex-col space-y-1">
                <div class="font-semibold text-base text-color_text leading-tight" style="color:{{ $nickname_color }}">
                    ...
                    <div class="text-xs font-normal text-color_text opacity-50 mt-1">{{$comment->created_at->diffForHumans()}}</div>
                </div>
            </div>
        </div>
        <div class="flex flex-wrap justify-end items-center content-center w-full {{--xl:w-[650px] md:w-[450px]--}}">
            <div class="rounded-lg ml-0 sm:ml-12 p-3 pb-0 bg-background bg-opacity-50 relative w-full">
                ...
                <div class="flex items-center justify-end space-x-2 opacity-75">
                    <div class="h-5 w-5 reply-button cursor-pointer" data-comment-id="{{ $comment->id }}" data-user-name="{{ $comment->user->name }}">
                        <x-icon name="response" fill="#252525"/>
                    </div>
                    <div class="like-comment-container flex" data-post-comment-id="{{ $comment->id }}" data-is-liked="{{ $isLikedComment ? 'true' : 'false' }}" data-is-processing="false">
                        <x-icon-with-text icon="like" fill="#ff3b3c" class="like-icon active" style="{{ $isLikedComment ? '' : 'display:none;' }}">
                            <span class="likes-count">{{ $comment->likes->count() }}</span>
                        </x-icon-with-text>
                        <x-icon-with-text icon="like" fill="#252525" class="like-icon inactive" style="{{ !$isLikedComment ? '' : 'display:none;' }}">
                            <span class="likes-count">{{ $comment->likes->count() }}</span>
                        </x-icon-with-text>
                    </div>
                </div>
