    ...
    <div class="">
        <div class="flex flex-row items-center flex-wrap space-x-2 space-y-2">
            @for ($i = 0; $i < 7; $i++)
                <div class="inline-block mt-2">
                    @if ($collectedToday)
                    <div class="{{ $collectedDays > $i ? 'bg-success' : 'border' }} {{ $collectedDays == $i ? 'border-dashed' : '' }} px-2 cursor-pointer rounded flex flex-col items-start">
                    @else
                    <div class="{{ $collectedDays > $i ? 'bg-success' : 'border' }} {{ $collectedDays == $i ? 'border-dashed' : '' }} px-2 cursor-pointer rounded flex flex-col items-start">  
                    @endif
                        {{-- <div>
                        ...
                        <div class="flex">
                            @switch($i)
                                @case(0)
                                    ...
                                    @break
                                @case(1)
                                    <x-icon-with-text icon="megaball" tooltip="{{__('profiles.megaball')}}">
                                        2
                                    </x-icon-with-text>
                                    @break
                                @case(2)
                                ...
                                @case(6)
                                    <x-icon-with-text icon="coins" tooltip="{{__('profiles.coins')}}">
                                        8
                                    </x-icon-with-text> <div class="mx-2 border"></div> 
                                    <x-icon-with-text icon="ultraball" tooltip="{{__('profiles.ultraball')}}">
                                        1
                                    </x-icon-with-text>
                                    @break
                            @default
        ...
        <form action="{{ route('collect.daily.reward') }}" method="POST" class="mt-6 flex">
            ...
            <div class="flex flex-col">
                ...
                @if ($collectedToday)
                    <div class="mt-1 text-xs text-content_text opacity-50" x-data="rewardTimer('{{ now() }}')"> {{__('interfaces.time_left')}}
                        <span x-text="timeLeft.hours"></span>:<span x-text="timeLeft.minutes"></span>:<span x-text="timeLeft.seconds"></span>
                    </div>
                @endif
            </div>
        </form>
        ...
        return {
            ...
            updateTimeLeft() {
                ...
                if (timeDiff >= 0) {
                    ...
                    this.timeLeft.minutes = Math.floor((timeDiff % (1000 * 60 * 60)) / (1000 * 60)).toString().padStart(2, '0');
