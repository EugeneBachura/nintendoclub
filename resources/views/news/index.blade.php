                            ...
                            <td class="px-5 py-4 whitespace-nowrap text-sm text-content_text">
                                @foreach(['en', 'ru', 'pl'] as $locale)
                                    @if($title = $news->getTranslation('title', $locale))
                                        @if ($locale == 'en')
                                            <a href="{{ route('news.show', ['alias' => $news->alias]) }}" class="text-indigo-600 hover:text-indigo-900">({{ strtoupper($locale) }}) {{ $title }}</a><br>
                                        @else
                                            <a href="{{ route('news.show.locale', ['alias' => $news->alias, 'locale' => $locale]) }}" class="text-indigo-600 hover:text-indigo-900">
                                                ({{ strtoupper($locale) }}) {{ $title }}
                                            </a><br>
                                        @endif
                                    @endif
                                @endforeach
