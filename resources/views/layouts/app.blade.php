...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased bg-background">
        ...
        <div class="min-h-screen min-w-[357px] bg-background text-color_text flex flex-col ">
            ...
            <div class="flex flex-1 justify-center space-x-4 px-2 sm:px-6">
                ...
                @endif
                @if(session('success'))
                    <div x-data="{ open: true }" x-show="open" class="bg-successfully text-successfully-text p-4 rounded-md shadow-lg absolute z-20 top-20">
                ...
                @isset ($errors)
                    @if($errors->any())
                    <div x-data="{ open: true }" x-show="open" class="bg-error text-error-text p-4 rounded-md shadow-lg absolute z-20 top-20">
                        <div class="flex justify-between items-start relative pr-2">
                            <div class="flex flex-col">
                ...
                <main class="flex-1 overflow-y-auto content">
                        <div class="py-8">
                            <div class="max-w-full min-w-[340px] w-[1200px]
                            @isset ($slim)
                            w-[800px]
                            @endisset
                             mx-auto">
        ...
        <div id="cookie-notification" style="display: none" class="items-center text-sm fixed bottom-0 left-0 w-full bg-background text-content_text z-10 p-2 sm:px-6 sm:py-3  justify-between">
            <p>
                {{__('interfaces.cookie')}}
                @if (app()->getLocale() == "en")
                <a href="{{route('terms') }}" class="hover:text-content_text-hover underline">{{__('titles.legal_information')}}</a>
                @else
                <a href="{{ localizedRoute('terms') }}" class="hover:text-content_text-hover underline">{{__('titles.legal_information')}}</a>
                @endif
            </p>
    ...
    <!-- Fonts -->
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600;700&family=Playfair+Display:wght@400;700&display=swap" rel="stylesheet"  type="text/css"
    media="print"
