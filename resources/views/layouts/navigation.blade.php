        ...
        <div class="flex justify-between h-20 items-center">
            <div class="flex flex-1">
                ...
                <div class="shrink-0 flex items-center">
                    ...
                    <div class="text-base flex items-center"><span
            ...
            <div class=" flex-1 justify-evenly text-base hidden sm:flex">
                @if (app()->getLocale() == 'en')
                    <a href="{{ route('news.showAll') }}"
                        class="flex content-center items-center hover:text-accent-hover">{{ __('titles.news') }}</a>
                    <a href="{{ route('post.showAll') }}"
                        class="flex content-center items-center hover:text-accent-hover">{{ __('titles.posts') }}</a>
                    <a href="{{ route('game.showAll') }}"
                        class="flex content-center items-center hover:text-accent-hover">{{ __('titles.games') }}</a>
                @else
                    <a href="{{ route('news.showAll.locale', ['locale' => app()->getLocale()]) }}"
                        class="flex content-center items-center hover:text-accent-hover">{{ __('titles.news') }}</a>
                    <a href="{{ route('post.showAll.locale', ['locale' => app()->getLocale()]) }}"
                        class="flex content-center items-center hover:text-accent-hover">{{ __('titles.posts') }}</a>
                    <a href="{{ route('game.showAll.locale', ['locale' => app()->getLocale()]) }}"
                        class="flex content-center items-center hover:text-accent-hover">{{ __('titles.games') }}</a>
                @endif
            </div>
            ...
            <div class="flex items-center content-center justify-end space-x-5 flex-1">
                @if (Auth::user())
                    <x-avatar src="{{ auth()->user()->avatar }}" size="w-10 h-10"
                        class="hidden sm:visible mr-2 sm:mr-0 sm:flex sm:items-center" />
                    ...
                    <div class="hidden sm:flex sm:items-center sm:ml-1.5">
                        <x-dropdown align="right" width="48">
                            <x-slot name="trigger">
                                <button
                                    class="inline-flex items-center py-2 border border-transparent text-base leading-4 font-medium rounded-md text-nav_text bg-nav hover:text-nav_text-hover focus:outline-none transition ease-in-out duration-150">
                    ...
                    <div class="-mr-2 flex items-center sm:hidden">
                        <button @click="open = ! open"
                            class="inline-flex items-center justify-center p-2 rounded-md text-nav_text hover:text-nav_text-hover hover:bg-nav-hover focus:outline-none focus:bg-nav-hover focus:text-nav_text-hover transition duration-150 ease-in-out">
